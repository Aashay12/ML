...
plt=test_data.Title.value_counts().plot('bar')

#REPLACING VALUES IN TITLE

test_data['Title'] = test_data['Title'].replace(['Dr', 'Rev', 'Col', 'Major', 'Countess', 'Sir', 'Jonkheer', 'Lady', 'Capt', 'Don','Dona'], 'Others')
...
#TESTING MODEL
gensub=pd.read_csv(r'C:\Users\Lenovo\Desktop\ML\Titanic\gender_submission.csv')
gensub['Prediction']=logreg_clf.predict(test_data)

#CHECKING ACCURACY

print('accuracy score:',accuracy_score(gensub['Survived'],gensub['Prediction']))

confusion_test=confusion_matrix(gensub['Survived'],gensub['Prediction'],labels=[1,0])
print(confusion_test)

report_test=classification_report(gensub['Survived'],gensub['Prediction'])
print(report_test)
